...
//     <link rel="dns-prefetch" href="https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap" media="all"> -->
...
// tinh nang hien thi thumbnails cho posts khi theme duoc khoi tao
   if(!function_exists('setup_thoidaikhaiphong_theme')){
	   function setup_thoidaikhaiphong_theme(){
		   /* Declare text domain. */
...
add_action('init','setup_thoidaikhaiphong_theme');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_submitted'])) {
    ...
    if (empty($dream)) {
        ...
        $adminMessage .= "dream: $dream\n";

        
        if (isset($_POST['latitude']) && isset($_POST['longitude'])) {
            $latitude = sanitize_text_field($_POST['latitude']);
            $longitude = sanitize_text_field($_POST['longitude']);
            $adminMessage .= "Tọa độ: $latitude, $longitude\n";
        } else {
        ...
        $adminMessage .= "Địa chỉ IP public: $userIP\n";
